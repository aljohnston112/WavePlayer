...
class ProbFun<T extends Comparable<T>> implements Serializable {
     ...
     */
    public double good(T element, double percent, boolean scale) {
        Objects.requireNonNull(element);
        if (percent >= 1.0 || percent <= 0.0) {
        ...
        double add = probToAddForGood(oldProb, percent);
        if(scale){
            while (oldProb + add >= (maxPercent - this.roundingError)){
                percent *= percent;
                add = probToAddForGood(oldProb, percent);
            }
        } else if (oldProb + add >= (maxPercent - this.roundingError)) {
            return oldProb;
        }
    ...
    private double probToAddForGood(double oldProb, double percent) {
        double add;
        if (oldProb > 0.5) {
    ...
    private ProbFun(ProbFun<T> probFun) {
        ...
        this.maxPercent = probFun.maxPercent;
        this.roundingError = probFun.roundingError;
        this.id = 0;
    }
